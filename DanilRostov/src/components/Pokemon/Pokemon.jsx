...
export default class Pokemon extends Component {
	...
	onCatch = () => {
		...
		const date = new Date();
		const dateString = '0' + date.getUTCDay() + '.' +
											'0' + date.getUTCMonth() + '.' +
											date.getUTCFullYear() + ' ' +
											date.getHours() + ':' +
											date.getMinutes() + ':' +
	...
	render() {
		...
		const btn = this.props.catchPossible ? <button disabled={btnDisabledValue} className="btn btn-success col-sm-4 col-lg-2" onClick={this.onCatch}>{btnValue}</button> : null;
